...
const SpaceGame = () => {
  ...
  const getRockDensity = useCallback((worldX, worldY) => {
    ...
    const n1 = noise(worldX * gameState.noiseScale, worldY * gameState.noiseScale);
    const n2 = noise(worldX * gameState.noiseScale * 2, worldY * gameState.noiseScale * 2) * 0.5;
    const n3 = noise(worldX * gameState.noiseScale * 4, worldY * gameState.noiseScale * 4) * 0.25;
  ...
  const generateRockChunk = useCallback((chunkX, chunkY) => {
    const gameState = gameStateRef.current;
    const chunk = {
      x: chunkX,
      y: chunkY,
  ...
  const getVisibleChunks = useCallback(() => {
    ...
    for (let dx = -gameState.renderDistance; dx <= gameState.renderDistance; dx++) {
      for (let dy = -gameState.renderDistance; dy <= gameState.renderDistance; dy++) {
        const cx = chunkX + dx;
        const cy = chunkY + dy;
        const key = `${cx},${cy}`;
  ...
  const digTunnel = useCallback((x, y) => {
    ...
    let heroInValidPosition = heroDistFromCenter <= gameState.sphereRadius || !isInRock(hero.x, hero.y);
    ...
    if (!canDig) {
      ...
      if (hasRockOverlap) {
        ...
        const distanceToClick = Math.sqrt((hero.x - x) ** 2 + (hero.y - y) ** 2);
    ...
    <div className="flex flex-col items-center gap-4 p-4 bg-gray-900 min-h-screen">
      <Card className="p-6 bg-gray-800 border-gray-700">
        ...
        <p className="text-gray-300 text-center mb-4 font-mono text-sm">
          Explore infinite procedural rock formations. The map generates as you venture further.
        </p>
